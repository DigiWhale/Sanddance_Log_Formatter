...
def plot_coordinates_on_mapbox(df, save_path, folder_path):
    ...
    if not df.empty:
        try:
            fig = px.scatter_mapbox(df, lat='rpi_lat', lon='rpi_lon', zoom=18, color_discrete_sequence=['#fd6bbe'], animation_frame='timestamp', center={'lat': df['rpi_lat'][0], 'lon': df['rpi_lon'][0]}, hover_data=["timestamp"])
